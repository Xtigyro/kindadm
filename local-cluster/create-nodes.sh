...
case "${NO_NODES}" in
  "1")
  CLUSTER_WRKS=(kind-"${NO_NODES}"-control-plane)
  ;;
  "[2-9]")
  CLUSTER_WRKS=(kind-"${NO_NODES}"-worker2)
  ;;
  *)
  echo -e "\e[32m\n\e[1mPass the number of the desired nodes - 1|2|3|4|5|6|7|8|9.\e[00m"
  exit 11
  ;;
esac

# Create kINd cluster
kind create cluster --config ./kind-config.yaml --name kind-"${NO_NODES}"
# Deploy MetalLB
